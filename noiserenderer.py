...
def render_chunk(chunk_id, start_frame, end_frame, input_path,
    ...
    try:
        ...
        if noise_type == "Scrolling":
            scrolling_noise_texture = np.random.randint(0, 256, (internal_h, internal_w), dtype=np.uint8)
        ...
        while current_frame < end_frame:
            ...
            if noise_type == "Random":
                ...
                active_noise = cv2.merge([active_gray, active_gray, active_gray]).astype(np.float32)
            else: # Scrolling Noise
                if scroll_direction == 'Down':
                    scrolling_noise_texture = np.roll(scrolling_noise_texture, shift=scroll_speed, axis=0)
                elif scroll_direction == 'Up':
                    scrolling_noise_texture = np.roll(scrolling_noise_texture, shift=-scroll_speed, axis=0)
                elif scroll_direction == 'Right':
                    scrolling_noise_texture = np.roll(scrolling_noise_texture, shift=scroll_speed, axis=1)
                elif scroll_direction == 'Left':
                    scrolling_noise_texture = np.roll(scrolling_noise_texture, shift=-scroll_speed, axis=1)
                active_noise = cv2.merge([scrolling_noise_texture, scrolling_noise_texture, scrolling_noise_texture]).astype(np.float32)
...
class ThumbnailGeneratorWindow(tk.Toplevel):
    def __init__(self, parent, video_path):
        ...
        self.internal_w = int(parent.target_width.get() // parent.internal_scale.get())
        ...
        self.static_noise = np.random.randint(0, 256, (self.internal_h, self.internal_w, 3), dtype=np.uint8)
    ...
    def setup_ui(self):
        self.lbl_preview = tk.Label(self, bg="#000", text="Loading...", fg="#555")
        self.lbl_preview.pack(pady=10, padx=10, expand=True, fill="both")
        ...
        vis_frame = tk.LabelFrame(self, text="Visibility Enhancer", bg="#1e1e1e", fg="#ccc")
        vis_frame.pack(fill="x", padx=10, pady=10)

        ttk.Checkbutton(vis_frame, text="Improve Visibility", variable=self.improve_visibility, command=self.update_preview).pack(side="left", padx=10)
        ...
        self.vis_slider.pack(side="left", padx=10)

        save_btn = ttk.Button(self, text="Save 16:9 Thumbnail (PNG)", command=self.save_thumbnail)
    ...
    def generate_noise_frame(self, original_frame):
        ...
        active_noise = np.random.randint(0, 256, (self.internal_h, self.internal_w, 3), dtype=np.uint8)
    ...
    def update_preview(self, event=None):
        ...
        if ret:
            ...
            if w > 1 and h > 1: img.thumbnail((w,h))
            self.tk_img = ImageTk.PhotoImage(image=img)
            self.lbl_preview.config(image=self.tk_img, text="")

    def save_thumbnail(self):
        if self.processed_frame is None:
            messagebox.showerror("Error", "No frame to save.", parent=self)
            return

        save_path = filedialog.asksaveasfilename(defaultextension=".png", filetypes=[("PNG files", "*.png")], title="Save Thumbnail As...", parent=self)
        ...
        canvas = np.random.randint(0, 256, (target_h, target_w, 3), dtype=np.uint8)
        ...
        try:
            ...
            messagebox.showerror("Error", f"Failed to save image: {e}", parent=self)
...
class NoiseRenderApp:
    ...
    def setup_ui(self):
        ...
        n_frame = tk.LabelFrame(self.root, text="Noise Options", bg="#121212", fg="#888")
    ...
    def open_thumbnail_generator(self):
        if not self.file_path.get() or not os.path.exists(self.file_path.get()):
            ...
            return
        ThumbnailGeneratorWindow(self, self.file_path.get())
    ...
    def update_ui(self):
        ...
        if previews:
            try:
                ...
                img.thumbnail((self.lbl_preview.winfo_width() - 10, self.lbl_preview.winfo_height() - 10))
    ...
    def run_logic(self, ffmpeg_exe):
        ...
        try:
            # --- MODIFIED LINE ---
            # Removed stdout and stderr redirection to prevent pipe buffer deadlock
            # and to show FFmpeg progress in the console.
            ...
            # The error message from FFmpeg will now appear in the console.
